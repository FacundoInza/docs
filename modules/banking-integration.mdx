...
<CardGroup cols={2}>
  ...
  <Card title="Via automation" icon="robot">
    For the remaining banks, Silver5 executes payments by operating the bank automatically from the desktop app with a phone (**Mobile**).
  </Card>
</CardGroup>
...
For banks that don't integrate via API, Silver5 executes payments by **operating the bank automatically** with the **Mobile** variant:

<CardGroup cols={1}>
  <Card title="Mobile Automatic Payments" icon="mobile-screen-button" href="/modules/automatic-payments/mobile">
    The Silver5 desktop app operates the bank app on **Android phones** (the BlueStacks emulator or real phones over USB). It covers banks that only operate from their mobile app, with one account per phone and several phones paying in parallel.
  </Card>
</CardGroup>
...
<Note>
  Is the bank you want to use not on the API list? Set up automatic payments with the [Mobile](/modules/automatic-payments/mobile) variant.
</Note>
...
<Frame caption="The integration is created in MANUAL mode and inactive">
  ![Summary of the banking integration creation in MANUAL mode](/images/automatic-payments/integration-manual-summary.png)
</Frame>
...
<Steps>
  <Step title="Open the order and tap Make Payment">
    ...
    <Frame caption="Order awaiting payment, with the Make Payment button">
      ![Order detail with the Make Payment button](/images/automatic-payments/order-realizar-pago.png)
    </Frame>
  </Step>
  ...
  <Step title="Enter the 2FA code and execute">
    ...
    <Frame caption="Payment confirmation with two-factor (2FA) code">
      ![Payment confirmation modal with 2FA code](/images/automatic-payments/confirm-payment-2fa.png)
    </Frame>
  </Step>
</Steps>
